...
void conditions::entry(){
  ...
  while(1) {
    ...
    if ((tmp1==4) && (tmp2<6) || (tmp5+sc_int<4>(tmp4.to_int())==6)) {
	out_value1.write(4);
    } else {
    ...
    if ((in_value1.read().to_uint()==4) && (sc_int<4>(in_value2.read().to_int())<6) || 
	(sc_int<4>(in_value4.read().to_int())+in_value5.read()==6)) {
	out_value2.write(4);
    } else {
    ...
    cond_tmp = (tmp1==4) && (tmp2<6) || (tmp5+sc_int<4>(tmp4.to_int())==6);
    if (cond_tmp) {
	out_value3.write(4);
    } else {
    ...
    tmp2a = 0;
    out_value4.write((sc_int<4>(tmp3.to_int())==4) && (tmp1<6) || 
    ...
    // therefor the temporary assignment
    tmp5 = tmp2.to_int();
    out_value5.write(sc_int<4>((in_value3.read().to_int())==4) &&
		     (sc_int<4>(in_value1.read().to_int())<6) || 
		     (in_value5.read()+sc_int<4>(in_value2.read().to_int())==6)?
		     0:tmp5); 
